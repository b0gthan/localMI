...
if not exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[Templates]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
	...
	Exec (@sCommand)
	SET @sCommand='INSERT INTO Templates (TemplateName, Description, IsDefault, SortOrder, RightName) VALUES (''Tabular Report without Header'', ''Start with a tabular report without a header or style. Perfect as a dashboard panel.'', 1, 2, '''')'
	Exec (@sCommand)
	...
	Exec (@sCommand)
	SET @sCommand='INSERT INTO Templates (TemplateName, Description, IsDefault, SortOrder, RightName) VALUES (''Bar Chart'', ''Start with a bar chart without header or style. Perfect as a dashboard panel.'', 0, 4, ''RB_BCT'')'
	Exec (@sCommand)
